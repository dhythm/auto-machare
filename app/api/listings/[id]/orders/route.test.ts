...
describe('orders API', () => {
  it('opens an order and walks it to completion', async () => {
    const created = await post('car-001', { message: '現金で' })
    expect(created.status).toBe(201)
    const order = await created.json()
    expect(order).toMatchObject({ status: 'requested', price: 2_180_000 })
    expect((await post('car-001', {})).status).toBe(409)
  ...
  it('maps failures', async () => {
    expect((await post('trk-005', {})).status).toBe(409)
    expect((await post('missing', {})).status).toBe(404)
    signInAs(demoSeller)
    expect((await post('car-001', {})).status).toBe(403)
    signInAs(demoAdmin)
    expect((await patch('nope', { status: 'cancelled' })).status).toBe(404)
    signInAs(null)
    expect((await post('car-001', {})).status).toBe(401)
