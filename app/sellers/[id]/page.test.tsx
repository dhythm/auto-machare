  ...
  getSellerProfile: vi.fn(async (id: string) =>
    id === 'demo-seller'
      ? {
          ...
          rating: 4.5,
          reviewCount: 2,
          listings: [await getListing('car-001'), await getListing('suv-002')],
          reviews: [
            {
              id: 'rv-1',
              listingId: 'car-001',
...
describe('SellerPage', () => {
  it('shows the seller name, rating, live listings and reviews', async () => {
    ...
    const listings = screen.getByRole('region', { name: '出品中の車両' })
    expect(within(listings).getAllByRole('link')).toHaveLength(2)
    expect(
      within(listings).getByRole('link', { name: /トヨタ プリウス Z/ }),
    ...
    expect(
      within(reviews).getByRole('link', { name: /トヨタ プリウス Z/ }),
    ).toHaveAttribute('href', '/listings/car-001')
  })
