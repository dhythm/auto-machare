...
export function CarrierProfileForm({
  ...
  contact?: FormContact
  initial?: CarrierProfileInput & { note?: string }
}) {
  ...
  const toggle = (key: 'vehicles' | 'serviceAreas', item: string) =>
    set(
      key,
  ...
  const submit = async (event: FormEvent) => {
    ...
    setError(undefined)
    setSaved(false)
    const parsed = validateCarrierProfile(values)
    if (!parsed.ok) {
    ...
    try {
      const response = await fetch('/api/transport/carrier-profile', {
        method: 'PUT',
        ...
        <div className="flex flex-wrap gap-4">
          {carrierVehicleTypes.map((vehicle) => (
            <CheckboxField
              ...
              onChange={() => toggle('vehicles', vehicle)}
            />
          ))}
        </div>
        {errors.vehicles && (
          <p className="mt-1 text-xs text-destructive">{errors.vehicles}</p>
        )}
      </fieldset>
      <fieldset>
        <legend className="mb-2 text-sm font-medium text-foreground">
