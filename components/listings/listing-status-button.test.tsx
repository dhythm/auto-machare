...
describe('ListingStatusButton', () => {
  it('withdraws a listed listing', async () => {
    ...
    render(<ListingStatusButton listingId="car-001" withdrawn={false} />)
    await userEvent
      .setup()
      .click(screen.getByRole('button', { name: '取り下げる' }))
    expect(fetchMock).toHaveBeenCalledWith(
      '/api/listings/car-001/status',
  ...
  it('republishes and shows the server error', async () => {
    vi.stubGlobal(
      'fetch',
      vi.fn(async () =>
        Response.json({ error: '進行中のリースがあります。' }, { status: 409 }),
      ),
    )
    render(<ListingStatusButton listingId="car-001" withdrawn />)
    await userEvent
      .setup()
      .click(screen.getByRole('button', { name: '再掲載する' }))
    expect(
      await screen.findByText('進行中のリースがあります。'),
    ).toBeInTheDocument()
  })
})
