...
async function completedLease() {
  const created = await requestLease((await getListing('car-001'))!, demoUser, {
...
describe('createReview', () => {
  it('lets the lessee review a completed lease and updates the seller rating', async () => {
    const before = (await getListing('car-001'))!.seller
    ...
    const after = (await getListing('car-001'))!.seller
    expect(after.reviews).toBe(35)
    expect(after.rating).toBe(4.7)
    expect((await getListing('suv-002'))!.seller.reviews).toBe(59)
