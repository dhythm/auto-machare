...
describe('configuredAccounts', () => {
  ...
  it('lists the demo personas after the environment accounts', () => {
    ...
    expect(personas.map((account) => account.id)).toContain('nakamura-motors')
...
describe('authenticate', () => {
  ...
  it('signs personas in with the shared persona password', () => {
    clearAccountEnv()
    vi.stubEnv('NODE_ENV', 'test')
    expect(authenticate('nakamura-motors@example.com', 'dev-persona')).toEqual({
      id: 'nakamura-motors',
      email: 'nakamura-motors@example.com',
      name: '中村モータース',
      role: 'user',
    })
    vi.stubEnv('DEMO_PERSONA_PASSWORD', 'shared-pass')
    expect(
      authenticate('nakamura-motors@example.com', 'dev-persona'),
    ).toBeUndefined()
    expect(
      authenticate('nakamura-motors@example.com', 'shared-pass'),
    ).toBeDefined()
  })
  ...
  it('keeps personas listed but not signable in production without a password', () => {
    clearAccountEnv()
    vi.stubEnv('NODE_ENV', 'production')
    expect(configuredAccounts().map((a) => a.id)).toContain('nakamura-motors')
    expect(
      authenticate('nakamura-motors@example.com', 'dev-persona'),
    ).toBeUndefined()
  })
