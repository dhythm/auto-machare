...
describe('requestOrder', () => {
  it('snapshots the price, notifies the seller, and blocks a second open order', async () => {
    const created = await request('car-001', demoUser, '現金で')
    ...
    const again = await request('car-001', demoUser)
  ...
  it('refuses the owner, listings without a sale price, and unpublished listings', async () => {
    ...
    const rentOnly = await request('trk-005')
...
describe('updateOrderStatus', () => {
  it('walks accept → deliver → complete and withdraws the listing at the end', async () => {
    ...
    expect((await getListing('car-001'))?.withdrawnAt).toEqual(
      expect.any(String),
    )
  })
...
describe('lease conversion', () => {
  it('creates a delivered order from the lease terms', async () => {
    ...
    expect(orders[0].order).toMatchObject({
      status: 'delivered',
      price: 2_180_000 - 201_600,
      sourceLeaseId: leaseId,
    })
    expect(orders[0].listing?.id).toBe('car-001')
    expect(
